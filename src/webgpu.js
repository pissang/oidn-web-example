...
import { readHDR } from 'oidn-web/lib/hdr.js';
import { WGPUFullQuadPass } from 'oidn-web/lib/WGPUFullQuadPass';

const denoisedCtx = document.getElementById('denoised').getContext('webgpu');
...
function createDisplayPass(device) {
...
initUNetFromURL('./rt_ldr_alb_nrm.tza', undefined, {
  aux: true,
  hdr: true
}).then((unet) => {
  Promise.all([
    ...
    ctx.putImageData(convertHDRDataToImageData(colorData), 0, 0);
